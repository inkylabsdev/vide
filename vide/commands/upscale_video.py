"""Upscale a video with FlashVSR, a real-time diffusion video super-resolution
model, and the `upscale-video` command that drives it.

Model and command share this one file (unlike `convert-depth-video`, which
splits model code into `vide/models/`) because FlashVSR has no
`transformers.pipeline`-style one-liner: it ships as a fork of the
`diffsynth` package (https://github.com/OpenImagingLab/FlashVSR) plus a
small LQ-projection module that the upstream repo keeps in its example
scripts rather than in the installable package. That module is vendored
below (`_build_lq_proj`, adapted from `examples/WanVSR/utils/utils.py`,
Apache-2.0) since it isn't importable any other way; everything else (the
diffusion transformer, VAE, and sparse attention) comes from `diffsynth`
itself, which must be installed per the FlashVSR README (clone the repo,
`pip install -e .`, plus the Block-Sparse-Attention CUDA extension) and is
only ever imported lazily, so loading the vide CLI stays fast and this
...
MODELS = {
    "flashvsr": "JunhaoZhuang/FlashVSR",
}
WEIGHT_FILES = (
...
def cli(
    ...
    if len(frames) < MIN_FRAMES:
        raise click.ClickException(
            ...
            f"(need at least {MIN_FRAMES}, got {len(frames)})"
        )

    scaled_w, scaled_h, target_w, target_h = compute_scaled_dims(width, height, scale)
    click.echo(
        f"Preparing {len(frames)} frame(s): {width}x{height} -> {target_w}x{target_h} ..."
    )
    lq_video, num_frames = prepare_lq_video(frames, scaled_w, scaled_h, target_w, target_h)

    device = models.pick_device()
    click.echo(f"Loading {model} on {device} ...")
    pipe = load(MODELS[model], weights_dir, device)
    ...
    written = 0
    try:
        for frame in tensor_to_frames(result):
