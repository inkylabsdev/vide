...
Ported from dashed/whisperx-subtitles-replicate, trimmed to the readable-cue
core (no translation, diarization, or neural sentence segmentation). Imports
only the standard library + pysbd, so it is unit-testable without the
torch/whisperx stack.
...
def _align_clause_words(
    ...
    identical to a positional ``word_data[ptr:ptr+len(tokens)]`` slice. It
    ...
    if not matched and tokens and start_ptr < n:
        ...
        # is never worse than blind counting.
        count = len(tokens)
...
def _segment_sentences(text: str, segmenter: pysbd.Segmenter | None) -> list[str]:
    """Split text into sentences: prefer pysbd where a segmenter was built,
    else a regex fallback."""
...
def split_at_sentence_end(
    ...
    sentences = _segment_sentences(text, segmenter)
